...
friends_data<- lineas |>
  ...
  rename(temporada=season,personaje=speaker,alegre=Joyful,
         asustado=Scared) |> 
  mutate(
    img=paste0("https://raw.githubusercontent.com/rladies/meetup-presentations_barranquilla/master/Tablas%20Rstudio/img/",
               gsub(' ','%20',paste0(personaje)),".webp",sep=""),
  ...
  reactable(
    ...
    defaultColDef = colDef(header = function(value) str_to_sentence(value),
      ...
      columns= list(
        ...
        personaje=colDef(align = "center",width = 160),
        temporada=colDef(align = "left",width = 120),
        lineas=colDef(minWidth=70,cell = color_tiles(data_tabla, bias= 0.3,colors=paleta)),
        alegre=colDef(minWidth=140, 
        ...
        details=function(index){
          ...
          reactable(nuevo_datos,compact = TRUE,
                    ...
                    columns=list(
                      img= colDef(name="", width=165),
                      temporada= colDef(show = F),
                      personaje=colDef(align="left", vAlign="center",minWidth = 160, width=160, 
                                       cell = function(value) {
                                         url<- friends_data |> ungroup()|> filter(personaje==value)|> select(img) |> distinct() |> as.character()
